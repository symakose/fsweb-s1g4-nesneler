...
const menuEleman2 = MenuElemaniOlustur("Çıtır Tavuk", 12, "Atıştırmalıklar");
const menuEleman3 = MenuElemaniOlustur("Mevsim Salata", 6, "Salatalar");
...
const burger = {
  ...
  indirim: function (kullanici) {
    if (kullanici === "öğretmen") {
      return this.fiyat * 0.75;
    } else if (kullanici === "öğrenci") {
...
let Ahmetinbildirimi;

for (let i = 0; i < degerlendirmeler.length; i++) {
  if (degerlendirmeler[i].isim === "Ahmet") {
    Ahmetinbildirimi = degerlendirmeler[i].geribildirim;
  }
}

console.log(Ahmetinbildirimi);
...
function DegerlendirmeEkle(
  ...
  const yeniDegerlendirme = {
    ...
    puan: 3.5,
    geribildirim:
      "Yemekler ılık servis edilse de mekan tasarımı ve ferahlığı cezbediciydi.",
  };
...
function AnahtardanDegerlendirmeAl(key6, indeks6) {
  console.log(
    key6[indeks6].isim +
      " isimli kişi " +
      key6[indeks6].puan +
      " puan verdi ve şunları yazdı: " +
      key6[indeks6].geribildirim
  );
  return;
}
